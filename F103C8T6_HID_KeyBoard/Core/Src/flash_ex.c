...
#define CustomConfig_ADDR ADDR_FLASH_PAGE_62
void ReadCustomData(void)
{
	KeyBuff[0] = *(__IO uint32_t *)(CustomConfig_ADDR);
	KeyBuff[1] = *(__IO uint32_t *)(CustomConfig_ADDR+1);
	KeyBuff[2] = *(__IO uint32_t *)(CustomConfig_ADDR+2);
	KeyBuff[3] = *(__IO uint32_t *)(CustomConfig_ADDR+3);
}
void CustomDataWriteToFlash(uint32_t DATA_64)
{
  ...
  EraseInitStruct.PageAddress = CustomConfig_ADDR;
...
//	DATA_32 = MAC_SAVE[3]<<24 | MAC_SAVE[2]<<16 | MAC_SAVE[1]<<8 | MAC_SAVE[0];
	
	if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, CustomConfig_ADDR, DATA_64) != HAL_OK)
	{
		Error_Handler();
	}
